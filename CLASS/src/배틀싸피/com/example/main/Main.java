package 배틀싸피.com.example.main;
import 배틀싸피.com.example.libraries.Bridge;

import java.util.*;

public class Main {
    private static String NICKNAME = "서울11반_김유정";
...
//////////////////////////////////////////////////////////////////////////////////
    static class Point {
		...
		public String toString() {
			return "Point [r=" + r + ", c=" + c + ", command=" + command + "]";
		}
    }
    
	static int normalBullet = 0;
	static int strongBullet = 0;
//////////////////////////////////////////////////////////////////////////////////
        ...
        while (gameData.length() > 0) {
            ...
            for (String key : allies.keySet()) {
                ...
                if (key.equals("A")) {
                    ...
                    normalBullet = Integer.parseInt(value[2]);
                    strongBullet = Integer.parseInt(value[3]);
            ...
            int[] myPosition = {-1, -1}; // 내 전차의 위치 가져오기
            for (int i = 0; i < mapData.length; i++) {
                for (int j = 0; j < mapData[i].length; j++) {
            ...
            int[] enemyPosition = {-1, -1}; // 적군의 포탑 위치 가져오기
            for(int i = 0; i < mapData.length; i++) {
            	for(int j = 0; j < mapData[i].length; j++) {
            	...
            	if(enemyPosition[0] > 0) break;
            }
            
            boolean enemyTank = false;
            int[] enemyTankPosition = {-1, -1}; // 적군의 탱크 위치 가져오기
            for(int i = 0; i < mapData.length; i++) {
            	for(int j = 0; j < mapData[i].length; j++) {
            		if(mapData[i][j].equals("E1")) {
            			enemyTankPosition[0] = i;
            			enemyTankPosition[1] = j;
            			enemyTank = true;
            			break;
            		}
            	}
            	if(enemyTankPosition[0] > 0) break;
            }
            
            boolean storage = false;
            int[] storagePosition = {-1, -1}; // 보급 시설 위치 가져오기
            for(int i = 0; i < mapData.length; i++) {
            	for(int j = 0; j < mapData[i].length; j++) {
            	...
            	if(storagePosition[0] > 0) {
            		break;
            	}
            }
            
            int[][] dir = {{-1, 0}, {1, 0}, {0, -1}, {0, 1}};
            String[] commandArr = {"U A", "D A", "L A", "R A"}; // 명령 배열
            
            // 보급시설이 있고 아직 보급시설을 방문하지 않았다면 보급시설로 이동하자, 3개만 얻자 일단...
            if(storage && strongBullet < 1 && enemyTank) {
        		boolean[][] isVisited = new boolean[mapData.length][mapData[0].length];
	            
	            Queue<Point> queue = new LinkedList<>();
	
	            queue.add(new Point(myPosition[0], myPosition[1], ""));
	            isVisited[myPosition[0]][myPosition[1]] = true;
	            
	            for(int d = 0; d < 4; d++) {
	            	int nr = myPosition[0] + dir[d][0];
	            	int nc = myPosition[1] + dir[d][1];
	            	
	            	if(nr >= 0 && nr < mapData.length && nc >= 0 && nc < mapData[0].length && (mapData[nr][nc].equals("G") || mapData[nr][nc].equals("F")) && !isVisited[nr][nc]) {
	            		queue.add(new Point(nr, nc, commandArr[d]));
	            ...
	            while(!queue.isEmpty()) {
	            	...
	            	for(int d = 0; d < 4; d++) {
	            		...
	            		if(nr >= 0 && nr < mapData.length && nc >= 0 && nc < mapData[0].length && (mapData[nr][nc].equals("G") || mapData[nr][nc].equals("F")) &&!isVisited[nr][nc]) {
	            			queue.add(new Point(nr, nc, curr.command));
	            			isVisited[nr][nc] = true;
	            		}
	            	}
	            	
	            	if(mapData[curr.r][curr.c].equals("F")) {
	            		output = curr.command;
	            		break;
	            	}
	            }
            ...
            if(codes.length > 0) {
            	for(int d = 0; d < 4; d++) {
                	...
                	if(nr >= 0 && nr < mapData.length && nc >= 0 && nc < mapData[0].length && mapData[nr][nc].equals("F")) {
                		switch(d) {
                		case 0:
                			output = "U ";
                			break;
            	...
            	for(int i = 0; i < code.length(); i++) {
            		int idx = code.charAt(i) - 'A';
            		int newIdx = (idx+9)%26;
            		
            		char newWord = alphabet[newIdx];;
            		password += newWord;
            	}
            ...
            // 보급시설이 없거나 이미 보급시설을 방문한 뒤라면 포탑으로 이동하자
            if((!storage || strongBullet >= 1) || !enemyTank) {
	            boolean[][] isVisited = new boolean[mapData.length][mapData[0].length];
	            ...
	            for(int d = 0; d < 4; d++) {
	            	...
	            	if(nr >= 0 && nr < mapData.length && nc >= 0 && nc < mapData[0].length && (mapData[nr][nc].equals("G") || mapData[nr][nc].equals("X")) && !isVisited[nr][nc]) {
	            		queue.add(new Point(nr, nc, commandArr[d]));
	            		isVisited[nr][nc] = true;
	            	} 
//	            		else if(nr >= 0 && nr < mapData.length && nc >= 0 && nc < mapData[0].length && normalBullet >= 1 && mapData[nr][nc].equals("T") && !isVisited[nr][nc]) {
//            			queue.add(new Point(nr, nc, commandArr[d]));
//            			isVisited[nr][nc] = true;
//            		} 
	            }      
...
//		            		else if(nr >= 0 && nr < mapData.length && nc >= 0 && nc < mapData[0].length && (mapData[nr][nc].equals("T") && normalBullet >= 1) &&!isVisited[nr][nc]) {
//	            			queue.add(new Point(nr, nc, curr.command));
//	            			isVisited[nr][nc] = true;
//	            		} 
		            		else if(nr >= 0 && nr < mapData.length && nc >= 0 && nc < mapData[0].length && (mapData[nr][nc].equals("E1") && strongBullet >= 1) &&!isVisited[nr][nc]) {
	            			queue.add(new Point(nr, nc, curr.command));
	            			isVisited[nr][nc] = true;
	            		} 
	            	}
	            	
	            	if(mapData[curr.r][curr.c].equals("X")) {
	            		output = curr.command;
	            		break;
	            	}
	            }
            ...
            for(int d = 0; d < 4; d++) {
            	...
            	if(nr >= 0 && nr < mapData.length && nc >= 0 && nc < mapData[0].length && (mapData[nr][nc].equals("X"))) {
            		switch(d) {
            		case 0:
            			output = "U F M";
            			break;
            	...
            	} else if (nr >= 0 && nr < mapData.length && nc >= 0 && nc < mapData[0].length && mapData[nr][nc].equals("E1") && strongBullet > 0) {
            		switch(d) {
            		case 0:
            			output = "U F S";
            			break;
